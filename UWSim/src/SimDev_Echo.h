...
public:
	...
	std::string info;
	//constructor
	SimDev_Echo_Config(std::string type_):SimulatedDeviceConfig(type_){}
};
...
class SimDev_Echo_ROSPublisher : public ROSPublisherInterface {
	...
	SimDev_Echo * dev;
public:
	SimDev_Echo_ROSPublisher(SimDev_Echo *dev, std::string topic, int rate): ROSPublisherInterface(topic,rate), dev(dev) {}

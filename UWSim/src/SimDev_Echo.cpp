...
SimDev_Echo_Factory::processConfig(const xmlpp::Node* node, ConfigFile * config){
...
SimDev_Echo_Factory::getInterface(ROSInterfaceInfo & rosInterface, std::vector<boost::shared_ptr<SimulatedIAUV> > & iauvFile){
	...
	for (size_t i=0;i<iauvFile.size();++i)
		for (size_t d=0;d<iauvFile[i]->devices->all.size();++d)
			if (iauvFile[i]->devices->all[d]->getType() == this->getType() && iauvFile[i]->devices->all[d]->name == rosInterface.targetName){
				ifaces.push_back(
					boost::shared_ptr<ROSInterface>( new SimDev_Echo_ROSPublisher(dynamic_cast<SimDev_Echo*>(iauvFile[i]->devices->all[d].get()), rosInterface.topic, rosInterface.rate) )
				);

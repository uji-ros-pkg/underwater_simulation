...
uint32_t CommsLights::numCommsLights = 0;

CommsLights::CommsLights(osg::ref_ptr<osg::Group> root) {
  sceneRoot = root;
  float ledRadio = 0.02;
  ...
  redLightSource = LightBuilder::createLightSource(
      numCommsLights, osg::Vec4(1.0f, 0.0f, 0.0f, 1.0f), att);
  redLightSource->getLight()->setLinearAttenuation(0.1);
  redLightSource->getLight()->setQuadraticAttenuation(0.1);
  uint32_t numLight = GL_LIGHT0 + numCommsLights % 8; // maximum: 8 lights!
  redLightNum = numLight;
  sceneRoot->getOrCreateStateSet()->setMode(numLight, osg::StateAttribute::ON);

  numCommsLights++;

  greenLightNum = numCommsLights;
  numLight = GL_LIGHT0 + numCommsLights % 8; // maximum: 8 lights!
  greenLightSource = LightBuilder::createLightSource(
      numCommsLights, osg::Vec4(0.0f, 1.0f, 0.0f, 1.0f), att);
...
void CommsLights::UpdateLetState(ledType type, bool on) {
  uint32_t num;
...
void CommsLights::StartAnimationTest() {
...
osg::ref_ptr<osg::Node> CommsLights::GetOSGNode() { return node.get(); }

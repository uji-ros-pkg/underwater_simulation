...
btHfFluid::btHfFluid(btScalar gridCellWidth, int minX, int maxX, int minY, int maxY, int depth, int height)
{
  m_eta = NULL;
  m_temp = NULL;
  m_ground = NULL;
  ...
  setGridDimensions(gridCellWidth, ceil(maxX - minX) / gridCellWidth, ceil(maxY - minY) / gridCellWidth);

  m_aabbMin = btVector3(minX, minY, depth);
  m_aabbMax = btVector3(maxX, maxY, height);
...
{
  return m_gridCellWidth * i + m_aabbMin.getX();
}

btScalar btHfFluid::lengthPos(int j) const
{
  return m_gridCellWidth * j + m_aabbMin.getX();
}
...
{
  m_eta[index] = height - m_ground[index];
...
void btHfFluid::getAabbForColumn(int i, int j, btVector3& aabbMin, btVector3& aabbMax)
{
  ...
  aabbMin = btVector3(widthPos(i), lengthPos(j), g);
  aabbMax = btVector3(widthPos(i + 1), lengthPos(j + 1), h);
...
void btHfFluid::foreachGroundTriangle(btTriangleCallback* callback, const btVector3& aabbMin, const btVector3& aabbMax)
{
  ...
  startNodeX = (int)((minX - m_aabbMin.getX()) * m_gridCellWidthInv);
  startNodeZ = (int)((minZ - m_aabbMin.getY()) * m_gridCellWidthInv);

  endNodeX = (int)((maxX - m_aabbMin.getX()) * m_gridCellWidthInv);
  endNodeZ = (int)((maxZ - m_aabbMin.getY()) * m_gridCellWidthInv);
...
void btHfFluid::foreachFluidColumn(btHfFluidColumnCallback* callback, const btVector3& aabbMin,
...
void btHfFluid::updateHeight(btScalar dt)
{
  for (int j = 1; j < m_numNodesLength - 1; j++)
  {
    for (int i = 1; i < m_numNodesWidth - 1; i++)
    {
      ...
      btScalar deta = -btScalar(0.5f) * m_eta[index] * dt * m_gridCellWidthInv
          * ((m_u[m_velocityIndex][index + 1] - m_u[m_velocityIndex][index])
              + (m_v[m_velocityIndex][index + m_numNodesWidth] - m_v[m_velocityIndex][index]));
      m_eta[index] += deta;
...
void btHfFluid::advectVelocityU(btScalar dt)
{
  for (int i = 1; i < m_numNodesWidth - 1; i++)
  {
    for (int j = 1; j < m_numNodesLength - 1; j++)
    {
      ...
      v += (m_v[m_velocityIndex][index] + m_v[m_velocityIndex][index + 1]
          + m_v[m_velocityIndex][index + m_numNodesWidth] + m_v[m_velocityIndex][index + m_numNodesWidth + 1])
          * btScalar(0.25);
...
void btHfFluid::advectVelocityV(btScalar dt)
{
  for (int i = 1; i < m_numNodesWidth - 1; i++)
  {
    for (int j = 1; j < m_numNodesLength - 1; j++)
    {
      ...
      u += (m_u[m_velocityIndex][index] + m_u[m_velocityIndex][index + 1]
          + m_u[m_velocityIndex][index + m_numNodesWidth] + m_u[m_velocityIndex][index + m_numNodesWidth + 1])
          * btScalar(0.25);
...
void btHfFluid::updateVelocity(btScalar dt)
{
  for (int j = 1; j < m_numNodesLength - 1; j++)
  {
    for (int i = 2; i < m_numNodesWidth - 1; i++)
    {
      ...
      m_u[m_velocityIndex][index] += m_gravity * dt * m_gridCellWidthInv
          * (m_height[m_heightIndex][index] - m_height[m_heightIndex][index - 1]);
    }
  }
  ...
  for (int j = 2; j < m_numNodesLength - 1; j++)
  {
    for (int i = 1; i < m_numNodesWidth - 1; i++)
    {
      ...
      m_v[m_velocityIndex][index] += m_gravity * dt * m_gridCellWidthInv
          * (m_height[m_heightIndex][index] - m_height[m_heightIndex][index - m_numNodesWidth]);
    }
  }
}
...
void btHfFluid::setAbsorbBoundaryLeft(btScalar dt)
{
  for (int j = 0; j < m_numNodesLength; j++)
  {
    ...
    m_height[m_heightIndex][indexL] = ((m_gridCellWidthInv * m_height[(m_heightIndex + 1) % 2][indexL + 1])
        + (dt * c * m_height[m_heightIndex][indexL + 1])) / (m_gridCellWidthInv + dt * c);
...
static btScalar rangeOverlap(btScalar lo1, btScalar hi1, btScalar lo2, btScalar hi2, btScalar& loOut, btScalar& hiOut)
{
  ...
  else if (lo2 >= lo1 && lo2 <= hi1 && hi2 >= lo1 && hi2 <= hi1)
  {
    hiOut = hi2;
    loOut = lo2;
    return hi2 - lo2;
...
btHfFluidColumnRigidBodyCallback::btHfFluidColumnRigidBodyCallback(btRigidBody* rigidBody, btIDebugDraw* debugDraw,
                                                                   btScalar density, btScalar floatyness)
{
...
static bool sphereVsAABB(const btVector3& aabbMin, const btVector3& aabbMax, const btVector3& sphereCenter,
                         const btScalar sphereRadius)
{
...
bool btHfFluidColumnRigidBodyCallback::processColumn(btHfFluid* fluid, int w, int l)
{
  ...
  for (int i = 0; i < m_buoyantShape->getNumVoxels(); i++)
  {
    ...
    if (sphereVsAABB(columnAabbMin, columnAabbMax, m_voxelPositionsXformed[i], m_buoyantShape->getVoxelRadius()))
    {
      ...
      {
        //btScalar massDisplacedWater = voxelVolume * m_density * m_floatyness;  <-- this density is object density, changed to water density(constant)
        btScalar massDisplacedWater = voxelVolume * 1027 * m_floatyness;
        btScalar force = massDisplacedWater * -fluid->getGravity();
        btScalar impulseMag = force * dt;
        //std::cout<<"Aquin: "<<impulseMag<<std::endl;
        btVector3 impulseVec = btVector3(btScalar(0.0f), btScalar(0.0f), btScalar(1.0f)) * impulseMag;
        //std::cout<<"Aquin: "<<impulseVec.x()<<" "<<impulseVec.y()<<" "<<impulseVec.z()<<" "<<std::endl;
//#define CENTER_IMPULSE_ONLY 1
  ...
  if (columnVolume > btScalar(0.0))
  {
    ...
    if (applyFluidVelocityImpulse)
    {
      ...
      //std::cout<<"Aquin: "<<impulse.x()<<" "<<impulse.y()<<" "<<impulse.z()<<" "<<std::endl;

...
btHfFluidRigidCollisionAlgorithm::btHfFluidRigidCollisionAlgorithm(const btCollisionAlgorithmConstructionInfo& ci,
                                                                   ...
                                                                   bool isSwapped) :
    btCollisionAlgorithm(ci), m_isSwapped(isSwapped), m_convexTrianglecallback(ci.m_dispatcher1, col0, col1, !isSwapped) // we flip the isSwapped because we are hf fluid vs. convex and callback expects convex vs. concave
{
...
void btHfFluidRigidCollisionAlgorithm::processGround(const btDispatcherInfo& dispatchInfo, btManifoldResult* resultOut)
{
  ...
  m_hfFluid->foreachGroundTriangle(&m_convexTrianglecallback, m_convexTrianglecallback.getAabbMin(),
                                   m_convexTrianglecallback.getAabbMax());
  resultOut->refreshContactPoints();
  // restore the buoyant shape
  m_rigidCollisionObject->setCollisionShape(tmpShape);
}

btScalar btHfFluidRigidCollisionAlgorithm::processFluid(const btDispatcherInfo& dispatchInfo, btScalar density,
                                                        btScalar floatyness)
{
  ...
  m_hfFluid->foreachFluidColumn(&columnCallback, m_convexTrianglecallback.getAabbMin(),
                                m_convexTrianglecallback.getAabbMax());
...
//#define OLD_WAY
  ...
  /*std::cout<<dt<<" "<<scaled_mu<<" "<<rb->getLinearVelocity().x()<<" "<<rb->getLinearVelocity().y()<<" "<<rb->getLinearVelocity().z()<<std::endl;
   std::cout<<dt<<" "<<scaled_mu<<" "<<rb->getAngularVelocity().x()<<" "<<rb->getAngularVelocity().y()<<" "<<rb->getAngularVelocity().z()<<std::endl;
   std::cout<<"COLLISIOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOON"<<std::endl;*/
  rb->applyCentralImpulse(dt * scaled_mu * -rb->getLinearVelocity());
  rb->applyTorqueImpulse(dt * scaled_mu * -rb->getAngularVelocity());
#endif
}

void btHfFluidRigidCollisionAlgorithm::processCollision(btCollisionObject* body0, btCollisionObject* body1,
...
btScalar btHfFluidRigidCollisionAlgorithm::calculateTimeOfImpact(btCollisionObject* body0, btCollisionObject* body1,
                                                                 const btDispatcherInfo& dispatchInfo,
                                                                 btManifoldResult* resultOut)
{
  return btScalar(1.0);

...
public://made process and extract methods public to be used in Simulated Devices implementations
  ...
  void processVisual(boost::shared_ptr<const urdf::Visual> visual, Link &link, std::vector<Material> &materials);
  void processJoint(boost::shared_ptr<const urdf::Joint> joint, Joint &jointVehicle,int parentLink,int childLink);
  int processLink(boost::shared_ptr<const urdf::Link> link, Vehicle &vehicle, int nlink, int njoint, std::vector<Material> &materials); //returns current link number
